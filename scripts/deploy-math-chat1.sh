...
DOMAIN="math.chat1.co"
REMOTE_BASE="/var/www/math.chat1.co"
RELEASE_ID="$(date +%Y%m%d-%H%M%S)"

usage() {
  cat <<'USAGE'
Usage: scripts/deploy-math-chat1.sh [options]
...
Options:
  ...
  --domain <domain>        Public domain for smoke checks (default: math.chat1.co)
  --remote-base <path>     Remote static root (default: /var/www/math.chat1.co)
...
LOCAL_RELEASE_DIR="/tmp/light-math-release-$RELEASE_ID"
...
if [[ "$SKIP_BUILD" -eq 0 ]]; then
  run "cd \"$ROOT_DIR/light-math-hub\" && npm run build"
  run "cd \"$ROOT_DIR/light-maths\" && npm run build:all"
else
...
APP_IDS="$(
  cd "$ROOT_DIR"
  node -e 'const fs=require("fs"); const apps=JSON.parse(fs.readFileSync("light-math-hub/src/data/apps.json","utf8")); console.log(apps.filter(a=>a.enabled&&a.listed).map(a=>a.id).join(" "));'
)"

if [[ -z "$APP_IDS" ]]; then
  echo "no enabled+listed apps found in light-math-hub/src/data/apps.json" >&2
  exit 1
fi

run "rm -rf \"$LOCAL_RELEASE_DIR\""
run "mkdir -p \"$LOCAL_RELEASE_DIR/apps\""
run "cp -R \"$ROOT_DIR/light-math-hub/dist/.\" \"$LOCAL_RELEASE_DIR/\""

for app_id in $APP_IDS; do
  if [[ ! -d "$ROOT_DIR/light-maths/dist/apps/$app_id" ]]; then
    echo "missing app build output: light-maths/dist/apps/$app_id" >&2
    exit 1
  fi
  run "cp -R \"$ROOT_DIR/light-maths/dist/apps/$app_id\" \"$LOCAL_RELEASE_DIR/apps/$app_id\""
done
