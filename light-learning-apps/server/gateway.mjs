...
const HOST =
  process.env.LIGHT_LEARNING_APPS_API_GATEWAY_HOST ??
  process.env.LIGHT_APPS_API_GATEWAY_HOST ??
  "0.0.0.0";
const PORT = Number(
  process.env.LIGHT_LEARNING_APPS_API_GATEWAY_PORT ??
    process.env.LIGHT_APPS_API_GATEWAY_PORT ??
    7060
);

const DEFAULT_IDLE_TTL_SEC = Number(
  process.env.LIGHT_LEARNING_APPS_API_IDLE_TTL_SEC ??
    process.env.LIGHT_APPS_API_IDLE_TTL_SEC ??
    900
);
const DEFAULT_STARTUP_TIMEOUT_MS = Number(
  process.env.LIGHT_LEARNING_APPS_API_STARTUP_TIMEOUT_MS ??
    process.env.LIGHT_APPS_API_STARTUP_TIMEOUT_MS ??
    20000
);
const SWEEP_INTERVAL_MS = Number(
  process.env.LIGHT_LEARNING_APPS_API_SWEEP_INTERVAL_MS ??
    process.env.LIGHT_APPS_API_SWEEP_INTERVAL_MS ??
    30000
);
const SHUTDOWN_GRACE_MS = Number(
  process.env.LIGHT_LEARNING_APPS_API_SHUTDOWN_GRACE_MS ??
    process.env.LIGHT_APPS_API_SHUTDOWN_GRACE_MS ??
    5000
);
const COOLDOWN_BASE_MS = Number(
  process.env.LIGHT_LEARNING_APPS_API_COOLDOWN_BASE_MS ??
    process.env.LIGHT_APPS_API_COOLDOWN_BASE_MS ??
    2000
);
const COOLDOWN_MAX_MS = Number(
  process.env.LIGHT_LEARNING_APPS_API_COOLDOWN_MAX_MS ??
    process.env.LIGHT_APPS_API_COOLDOWN_MAX_MS ??
    60000
);
const AUTH_TOKEN =
  process.env.LIGHT_LEARNING_APPS_API_AUTH_TOKEN ?? process.env.LIGHT_APPS_API_AUTH_TOKEN ?? "";
const IDLE_TTL_OVERRIDE_HEADER = "x-light-idle-ttl-sec";
const MAX_IDLE_TTL_OVERRIDE_SEC = parsePositiveInt(
  process.env.LIGHT_LEARNING_APPS_API_MAX_IDLE_TTL_OVERRIDE_SEC ??
...
const ALLOWED_COMMANDS = new Set(
  (
    process.env.LIGHT_LEARNING_APPS_API_ALLOWED_COMMANDS ??
    process.env.LIGHT_APPS_API_ALLOWED_COMMANDS ??
    "npm,node,python,python3,uv,uvicorn"
  )
    .split(",")
    .map((item) => item.trim())
    .filter(Boolean)
