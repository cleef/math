...
const UNFOLDED_LAYOUT: Record<FaceId, { x: number; y: number; r: number; z: number; s: number }> = {
  front: { x: 0, y: 0, r: 0, z: 20, s: 1 },
  up: { x: 0, y: -90, r: 0, z: 18, s: 1 },
  down: { x: 0, y: 90, r: 0, z: 18, s: 1 },
  left: { x: -90, y: 0, r: 0, z: 18, s: 1 },
  right: { x: 90, y: 0, r: 0, z: 18, s: 1 },
  back: { x: 180, y: 0, r: 0, z: 16, s: 1 }
};
...
function evaluateCuts(cutEdges: string[]): Evaluation {
  ...
  if (success) {
    message = "SUCCESS: This cut plan can fully unfold. Click Validate Unfold to preview.";
...
function CubeCutExperimentPage() {
  ...
  const runScissorSimulation = () => {
    ...
    void (async () => {
      ...
      try {
        ...
        setPlannedCuts([]);
        setActiveEdgeId(null);
        setStatusText(evaluateCuts(nextCuts).message);
      } finally {
        if (runToken === runTokenRef.current) {
          setIsCutting(false);
        }
  ...
  const verifyUnfold = () => {
    ...
    const runToken = ++runTokenRef.current;
    setIsUnfolding(true);
    setStatusText("Validating unfoldability...");
    setUnfoldProgress(0);

    void (async () => {
      try {
        await animateProgress(1900, (progress) => {
          if (runToken === runTokenRef.current) {
            setUnfoldProgress(progress);
          }
        });
        if (runToken === runTokenRef.current) {
          setStatusText("SUCCESS: Validation complete.");
        }
      } finally {
        if (runToken === runTokenRef.current) {
          setIsUnfolding(false);
    ...
    <section className="workspace">
      <article className="panel panel--cube">
        <div className="panel__head">
          ...
          <div className="status-pill">{isCutting ? "Scissors Running" : "Ready"}</div>
        </div>
      ...
      <article className="panel panel--result">
        ...
        <div className="net-stage" aria-label="Cube unfold validation animation">
          {FACE_IDS.map((faceId) => {
            const from = FOLDED_LAYOUT[faceId];
            const to = UNFOLDED_LAYOUT[faceId];
            const x = lerp(from.x, to.x, unfoldProgress);
            const y = lerp(from.y, to.y, unfoldProgress);
            const r = lerp(from.r, to.r, unfoldProgress);
            const s = lerp(from.s, to.s, unfoldProgress);
            const z = Math.round(lerp(from.z, to.z, unfoldProgress));
            ...
            return (
              <div key={faceId} className={`net-face net-face--${faceId}`} style={style}>
                {FACE_LABELS[faceId]}
              </div>
            );
          })}
        ...
        <div className="button-row">
          <button
            type="button"
            onClick={verifyUnfold}
            disabled={!evaluation.success || isUnfolding || isCutting}
          >
            Validate Unfold
          </button>
        </div>
...
function CubeNetsDemoPage() {
  ...
  const cutSingleEdge = (edgeId: string) => {
    ...
    void (async () => {
      try {
        ...
        const nextCuts = [...cutEdgesDone, edgeId];
        setCutEdgesDone(nextCuts);
        setActiveEdgeId(null);
        setActiveCutProgress(0);
        if (nextCuts.length < 3) {
  ...
  const finalWidth = (selectedNet.bounds.maxX - selectedNet.bounds.minX + 1) * finalCellSize + 24;
  const finalHeight = (selectedNet.bounds.maxY - selectedNet.bounds.minY + 1) * finalCellSize + 24;
    ...
    <section className="workspace workspace--nets">
      ...
      <article className="panel panel--nets-detail">
        ...
        <section className="demo-block">
          ...
          <div className="cut-list">
            {selectedNet.cutEdges.map((edgeId) => (
              <span
                key={edgeId}
                className={`cut-tag ${cutEdgesDone.includes(edgeId) ? "cut-tag--done" : ""}`}
              >
        ...
        <section className="demo-block">
          ...
          <div className="button-row">
            <button type="button" className="btn-muted" onClick={resetCuts} disabled={isCuttingStep}>
              Reset Cuts
            </button>
          </div>
                ...
                return (
                  ...
                  >
                    ...
                    <input
                      ...
                      disabled={!openable || isCuttingStep}
                    />
                  </label>
                );
              })}
        ...
        <section className="demo-block">
          ...
          >
            {FACE_IDS.map((faceId) => {
              const cell = selectedNet.facePositions[faceId];
              return (
                <div
                  key={`final-${selectedNet.id}-${faceId}`}
                  className={`final-face net-face--${faceId}`}
                  style={{
                    left: `${(cell.x - selectedNet.bounds.minX) * finalCellSize + 12}px`,
                    top: `${(cell.y - selectedNet.bounds.minY) * finalCellSize + 12}px`
                  }}
                >
                  {FACE_LABELS[faceId]}
                </div>
    ...
    <main className="surface-lab">
      <header className="hero">
        ...
        <p className="hero__subtitle">
          {page === "cut"
            ? "Page 1: interactive edge-cut lab. Select edges, simulate scissors, then validate unfolding."
            : "Page 2: all 11 cube nets. Each includes cut edges, step-by-step scissor unfolding, and final shape."}
        </p>
      ...
      {page === "cut" ? <CubeCutExperimentPage /> : <CubeNetsDemoPage />}
    </main>
  );
}
