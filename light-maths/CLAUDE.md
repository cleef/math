...
A monorepo of interactive educational math applications. Each app is a standalone React + Vite + TypeScript project that builds to `dist/apps/<id>/` and is served by **light-math-hub** (a separate repo).
...
**light-math-hub** (separate repo) loads apps by fetching:
1. `/apps/<id>/lesson-spotlight.html` (preferred)
2. `/apps/<id>/game-spotlight.html` (fallback)

Apps register in `light-math-hub/src/data/apps.json` with a matching `id`.
