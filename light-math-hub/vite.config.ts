...
export default defineConfig(({ mode }) => {
  ...
  const appsHost = env.LIGHT_APPS_HOST_URL ?? "http://localhost:5174";
  const appsApiGateway = env.LIGHT_APPS_API_GATEWAY_URL ?? "http://localhost:7060";
  const idleTtlSecHeader = resolveIdleTtlSeconds(env.LIGHT_APPS_IDLE_TIMEOUT_MINUTES);

  return {
    plugins: [react()],
    server: {
      host: true,
      allowedHosts: ["light", "localhost", "127.0.0.1", "math.chat1.co"],
      proxy: {
